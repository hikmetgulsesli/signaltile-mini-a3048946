...
- SignalTile Mini turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page utility called SignalTile Mini. It should show four signal tiles, let the user cycle each tile between ok/warn/down with a stable button, persist state in localStorage, expose window.app state/actions, include one visible h1, and export a JSON snapshot. Keep it static HTML/CSS/JS onl…
...
- Record Operations - SignalTile Mini
- Record Editor - SignalTile Mini
- Insights - SignalTile Mini

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Record Operations - SignalTile Mini
...
SCREEN_SPEC_2:
- exact_screen_title: Record Editor - SignalTile Mini
...
SCREEN_SPEC_3:
- exact_screen_title: Insights - SignalTile Mini
